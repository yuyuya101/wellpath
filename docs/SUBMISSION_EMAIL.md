...
```bash
pnpm install
pnpm verify            # typecheck + lint + 62 个单测/集成测试 + 构建
...
- 自动化测试：Vitest 62 个（领域/契约/API/业务闭环，含减重·维持·增重三方向与节奏、分屏部分草稿合法但非法值拦截、增重 BMI25 上限、会员过期回退免费、锁定字段清单一致性、会员专属 AI 洞察的非会员 402 拦截/本地模型不可达兜底/生产环境不空等超时等权限与输入边界）+ Playwright 1 条用例 × 桌面/移动双视口，全部通过；
